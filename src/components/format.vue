      ...
      <el-card class="box-card">
        <template #header>
          <div class="card-header">
            <span>
              本工具用于将输入文本按权利要求书或者说明书的格式对替换词进行序号标记。
            </span>
          </div>
        </template>
        ...
        <div class="text item">
          ...
          的模式，只需要输入替换词后按
          <mark>回车键</mark>
          <IconEnterKey theme="outline" size="14" fill="#333" />
          将自动添加数字序号，重复操作即可。或者也可以按
          <mark>空格隔开替换词</mark>
          ，例如：
          <b>底板 弹簧 螺杆</b>
        </div>
      ...
      <el-card class="box-card">
        <div>
          ...
          <div id="submit-bottom">
            <el-button type="primary" @click="transform(1)">
              权利要求书
            </el-button>
...
<script setup>
import { EnterKey as IconEnterKey } from '@icon-park/vue-next';
...
function autoformat(value) {
  ...
  if (value.includes('1、')) {
    ...
    for (var i = 0; i < arr.length; i++) {
      smartinput.value +=
        (lastNumber + i + 1).toString() + '、' + arr[i].trim() + '；';
    }
    return;
  }
  // 按空格区分 自动添加序号
...
function transform(type) {
  let real_input =
    smart.value == '1' ? smartinput.value.trim() : input.value.trim();
  if (real_input == '') {
  ...
  if (
    real_input.match(/^([a-zA-Z0-9]+、[\u4e00-\u9fa5a-zA-Z0-9]+；)+$/) == null
  ) {
  ...
  if (type == 2) {
    let unused_words = words.filter(
      w => !origin.value.includes(w.split('、')[1])
    );
    if (unused_words.length > 0) {
      ElMessageBox.alert(
        '说明书中未出现标记：' + unused_words.join('；') + ' 请检查并修改',
        '提示',
        {
          confirmButtonText: '好的',
        }
...
function insertWord(words_list, word) {
  ...
  for (let i = 0; i < words_list.length; i++) {
    if (words_list[i].content.includes(word.content)) {
      // 之前word已经按长度排序过，所以这里只需要单向判断
      need_insert = false;
...
function replaceSubString(
  ...
  parent_words_list = parent_words_list.sort(
  ...
  for (let i = 0; i < parent_words_list.length; i++) {
    let marker =
      '###START###' +
      btoa(unescape(encodeURIComponent(parent_words_list[i].content))) +
      '###END###';
    contentWithMarker = replaceWithMarker(
      contentWithMarker,
      parent_words_list[i].content,
      marker
    );
  }
  let resultWithMarker = replaceString(
    substr,
    targetid,
    contentWithMarker,
    type,
