<template>
  <el-row>
    ...
    <el-col :span="18">
      ...
      <el-card class="box-card">
        <template #header>
          <div class="card-header">
            <span>本工具用于将输入文本按权利要求书或者说明书的规则进行格式检查。</span>
          </div>
        </template>
        <div class="card-header">
      ...
      <el-card>
        <div v-if="filetype == '1'">
          <div class="checkbox-header">步骤二：选择 <mark>权利要求书</mark> 的格式检查项</div>
          <div class="checkbox-content">
            <el-checkbox v-model="checkAll" :indeterminate="isIndeterminate" @change="handleCheckAllChange">
              <div v-if="checkAll">
                <b class="helpinfo">全部启用</b><IconCheckOne theme="outline" size="14" fill="#7ed321"/>
              </div>
              <div v-else-if="checkedRules.length == 0">
                <b class="helpinfo">啥都不用</b><IconAttention theme="outline" size="14" fill="#d0021b"/>
              </div>
              <div v-else>
                <b class="helpinfo">部分启用</b><IconInfo theme="outline" size="14" fill="#f5a623"/>
              </div>
            </el-checkbox>
            <el-checkbox-group v-model="checkedRules" @change="handleCheckedRulesChange">
              <el-checkbox style="display: block;" v-for="rule in rules" :key="rule" :label="rule">
                <el-tooltip
                  class="box-item"
                  effect="dark"
                  :content="rule.help"
                ...
                <div v-if="rule.name=='refHasBase'">
                  ...
                  <span><el-input-number v-model="refWidth" size="small" :min="2" :max="8" step-strictly @click.stop.prevent @change="handleRefWidthChange"/></span>
                </div>
                <div v-else>
                  <span>{{ rule.info }}</span>
                </div>
        ...
        <div v-else>
          <div class="checkbox-header">步骤二：选择 <mark>说明书</mark> 的格式检查项</div>
          <div class="checkbox-content">
            ...
            <el-checkbox-group v-model="checkedRules2" @change="handleCheckedRulesChange2">
              <el-checkbox style="display: block;" v-for="rule in rules2" :key="rule" :label="rule">
                <el-tooltip
                  class="box-item"
                  effect="dark"
                  :content="rule.help"
...
export default {
  ...
  methods: {
    remountCodeMirror(){
      // 获取codemirror中的文本内容
      let spans = document.querySelectorAll('.CodeMirror-code .CodeMirror-line [role="presentation"]')
      ...
      return (text, callback) => {
        ...
        this.textlintKernel
          ...
          .then(result => {
            ...
            const lintErrors = lintMessages.map(this.convertTextlintMessageToCodeMirror);
            callback(lintErrors);
          })
          .catch(error => {
            console.error(error);
    ...
    handleCheckedRulesChange2(value) {
      ...
      this.isIndeterminate2 = checkedCount > 0 && checkedCount < this.rules2.length
      this.updateEnabledRules();
    },
    mountCodeMirror() {
