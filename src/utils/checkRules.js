    ...
    [Syntax.Document](node) {
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        if (match) {
          ...
          if (dict.has(number)) {
            report(node, new RuleError("存在重复的序号：" + number.toString(), {
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        if (match) {
          ...
          if (found_number - number != 1) {
            if (number == 0) {
              report(node, new RuleError("首序号应从1开始，而不是：" + found_number.toString(), {
                index: intext_position
              }))
            } else {
              report(node, new RuleError("不连续的序号：" + found_number.toString(), {
                index: intext_position
              }));
            }
          }
...
export const extraSpace = function(context) {
  ...
  return {
    [Syntax.Document](node) {
      ...
      dict.forEach((v, k) => {
        if (k.toString() != "0") { // 非首行开头不能存在空格
          if (v.content.startsWith(" ")) {
            report(node, new RuleError("行首存在多余的空格", {
              index: v.position
            }));
          }
        }
        ...
        let c_content = ""
        if (c_without_number.startsWith("、") || c_without_number.startsWith("．")) {
          c_content = c_without_number.substring(1)
          if (c_content.includes(" ")) {
            report(node, new RuleError("存在多余的空格", {
              index: v.position + c_content.indexOf(" ") + k.toString().length + 1
            }));
          }
          ...
          if (c_content.includes(" ")) {
            report(node, new RuleError("存在多余的空格", {
              index: v.position + c_content.indexOf(" ") + k.toString().length + shift
            }));
          }
        }
      })
...
export const repeatPeriod = function(context) {
  ...
  return {
    [Syntax.Document](node) {
      ...
      dict.forEach((v, k) => {
        if (k.toString() == "0") { // 首行不检查
          return
        }
        let c = v.content.replace(/\n*$/, "")
        if (!c.endsWith("。")) {
          report(node, new RuleError("本条没有以句号结尾", {
            index: v.position
          }));
        }
        let r = new RegExp("。", "g")
        ...
        if (count > 1 ) {
          report(node, new RuleError("本条包含有多个句号", {
            index: v.position
          }));
        }
      })
      ...
      dict.forEach((v, k) => {
        ...
        for(let i = 0; i < c.length-1; i++) {
          if (punctuation.includes(c[i]) && punctuation.includes(c[i+1])) {
            if (c[i] == c[i+1]) {
              report(node, new RuleError("本条存在重复的标点符号：" + c[i], {
                index: v.position
              }));
            } else if ("。；：“！？{【（(‘".includes(c[i])){
              report(node, new RuleError("本条存在连续的标点符号" + c[i] + "与" + c[i+1], {
                index: v.position
              }));
            }
          }
          ...
          for (let i = 0; i < elementLength; i++) {
            for (let j = 1; j < Math.floor(elementLength / 2) + 1; j++) {
              if ((i + 2 * j) <= elementLength) {
                if (element.slice(i, i+j) == element.slice(i+j, i+ 2*j) && isNaN(element.slice(i, i+j)) ) { // 连续的数字可能是标号，如11
                  report(node, new RuleError("本条存在连续的词：" + element.slice(i, i+j), {
                    index: v.position
                  }));
                }
              }
    ...
    [Syntax.Document](node) {
      ...
      dict.forEach((v, k) => {
        ...
        if (c.match(reg_with_to)) {
          if (!c.includes("任一项")) {
            report(node, new RuleError("本条权利要求范围应加任一项限定词", {
              index: v.position
            }));
          } else {
          ...
          if (c.includes("任一项")) {
            report(node, new RuleError("本条权利要求\"任一项\"引用的数字要以 \"-\" 或 \"至\" 或 \"或\" 连接", {
              index: v.position
            }));
          }
          if (c.includes("权利要求")) {
        ...
        if (!isSubset(ref_number_set, appeared_number_set)) {
          report(node, new RuleError("本条权利要求引用的序号存在错误", {
            index: v.position
          }));
        }
      ...
      dict.forEach((v, k) => {
        ...
        for ( let i = 0; i < matches.length; i++) {
          ...
          if ( matches[i].content.length < findWidth ){
            if (ref_number_list.length > 0) { // 有引用先判断是否在引用号条目中出现
              found = findByRefList(dict, ref_number_list, matches[i].content, 100)
            }
            // 没有引用或者引用中没有匹配到那么只能判断是否在本条目之前出现过
            // 或者在上一条独权条目中出现过
            let pre_content = c.substring(0, matches[i].index)
            if (pre_content.includes(matches[i].content) || line_independent.includes(matches[i].content)) {
              found = true;
            }
            if (!found) {
              report(node, new RuleError("所述的：\"" + old_content + "\" 找不到引用基础", {
          ...
          while ( matches[i].content.length >= findWidth && !found ) {
            ...
            matches[i].content = old_content.substring(0, matches[i].content.length-1)
          }
          if (!found) {
            report(node, new RuleError("所述的：\"" + old_content + "\" 找不到引用基础", {
              index: v.position + matches[i].index
